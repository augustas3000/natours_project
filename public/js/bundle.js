...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYXVndXN0YXMzayIsImEiOiJja2F3bHg4ODczdnl5MzFwNjh3M3Q5YWxrIn0.akcQTLD8OaXDavkqgoCM_A';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // extend bounds to include the location in question
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert(); //   the loggin request will be followed by either success/error
  ...
  markup.innerHTML = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>").trim();
  document.querySelector('body').insertAdjacentElement('afterbegin', markup); //   after showing, we hide alerts after 5 secs
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = "http://localhost:3000/api/v1/users/".concat(type === 'password' ? 'update-my-password' : 'update-my-details');
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_51GuFdsJ6KVh1TT4rdoGhSZfrNLg28W3Mibag1qOMnhx3ryCVOYNCnf8iGP5he9geGPiZu9bZBig3UV3VlzjF2nUm00KvrkjiBX');
...
var userDetailsForm = document.querySelector('.form-user-data');
var passwordChangeForm = document.querySelector('.form-user-password');
var bookBtn = document.getElementById('book-tour'); // only if mapBox elemenet is present on page, thenget the locations
// and display map:
...
if (userDetailsForm) {
  userDetailsForm.addEventListener('submit', function (e) {
    e.preventDefault(); // programatically recreate the multipart form data(files uploads)
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 7:
              // clear password fields, once update finishes
